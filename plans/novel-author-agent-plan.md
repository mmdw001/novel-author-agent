...
### 第二阶段：五层记忆系统 + 层级摘要管道
- [ ] 2.1 实现热记忆管理（滑动窗口 + 自动更新）
- [ ] 2.2 实现温记忆管理（章节摘要自动生成）
- [ ] 2.3 实现故事圣经核心数据结构
- [ ] 2.4 实现上下文构建器（组装多层注入）
- [ ] 2.5 实现 Token 预算计算与截断
- [ ] 2.6 实现层级摘要管道（场景→章节→弧线→卷的逐级压缩）
- [ ] 2.7 实现摘要自动触发机制（场景写完→自动更新上层摘要）

### 第三阶段：世界书引擎
- [ ] 3.1 实现世界书数据模型和 CRUD API
- [ ] 3.2 实现世界书引擎核心（关键词扫描 + 优先级排序 + 排除过滤）
- [ ] 3.3 实现角色→世界书的自动同步
- [ ] 3.4 实现世界书管理页面
- [ ] 3.5 实现世界书注入预览面板

### 第四阶段：核心写作功能
- [ ] 4.1 实现项目管理（CRUD + 列表 + 仪表盘）
- [ ] 4.2 实现卷/弧线/章节层级结构管理
- [ ] 4.3 实现写作编辑器（Tiptap 集成）
- [ ] 4.4 实现 AI 助手面板
- [ ] 4.5 实现 Prompt 模板系统
- [ ] 4.6 实现 AI 续写功能（含完整上下文注入）
- [ ] 4.7 实现 AI 扩写/润色功能
- [ ] 4.8 实现 AI 新章节生成

### 第五阶段：场景规划与多分支续写
- [ ] 5.1 实现场景数据模型（Scene CRUD）
- [ ] 5.2 实现场景规划 UI（章节内场景列表）
- [ ] 5.3 实现 AI 从大纲生成场景计划
- [ ] 5.4 实现多分支续写（2-3个选项生成）
- [ ] 5.5 实现分支选择 UI（对比视图）

### 第六阶段：批量写作管道
- [ ] 6.1 实现 BatchWriteSession 数据模型
- [ ] 6.2 实现批量章节计划生成
- [ ] 6.3 实现批量 AI 写作（5-10章并发）
- [ ] 6.4 实现批量审阅 UI（章节列表 + 逐章审阅）
- [ ] 6.5 实现批量反馈 → 批量修正循环
...
- [ ] 7.4 实现伏笔管理看板
- [ ] 7.5 实现时间线可视化
- [ ] 7.6 实现母题数据模型（Motif/MotifAppearance）
- [ ] 7.7 实现母题追踪 UI（出现列表 + 强度热力图 + 频率趋势）
- [ ] 7.8 实现 AI 自动检测母题出现（写作后扫描识别新出现）

### 第八阶段：沉浸式写作系统
- [ ] 8.1 实现 POV 角色数据扩展（角色声音/思维方式/知识边界字段）
- [ ] 8.2 实现知识边界管理器 KnowledgeBoundaryManager
- [ ] 8.3 实现 StoryFact 追踪系统（谁在什么章节知道了什么）
- [ ] 8.4 实现沉浸式角色 Prompt 模板
- [ ] 8.5 实现多 POV 场景切换支持
- [ ] 8.6 实现知识边界过滤（拦截 AI 使用角色不该知道的信息）
- [ ] 8.7 实现默认提示词的沉浸式升级

### 第九阶段：作者提示词系统
- [ ] 9.1 实现 AuthorPrompt 数据模型和 API
- [ ] 9.2 实现默认作者提示词（沉浸式版本，含模板变量）
- [ ] 9.3 实现提示词编译器 PromptCompiler（变量替换 + 无变量追加）
- [ ] 9.4 实现提示词编辑器页面（双栏：编辑+实时预览）
- [ ] 9.5 实现提示词版本管理（历史版本回溯）
- [ ] 9.6 整合提示词编译器到上下文构建器（与记忆系统+沉浸式系统联动）

### 第十阶段：一致性保障
- [ ] 10.1 实现一致性检查引擎
- [ ] 10.2 实现定期自动检查（每10章触发）
- [ ] 10.3 实现问题报告与追踪
- [ ] 10.4 实现风格指南管理
- [ ] 10.5 实现批量修正建议

### 第十一阶段：结构分析与世界书自动生成
- [ ] 11.1 实现结构分析引擎（角色出场/张力/情节线/伏笔）
- [ ] 11.2 实现结构分析仪表盘 UI
- [ ] 11.3 实现自动预警系统（角色失踪/伏笔过期/情节线停滞）
- [ ] 11.4 实现新实体检测（AI 写作后扫描）
- [ ] 11.5 实现自动世界书条目草稿生成
- [ ] 11.6 实现写作模式切换（草稿/润色/聚焦）

### 第十二阶段：修订工作流与章节状态管线
- [ ] 12.1 实现章节状态数据模型（planned/writing/draft/reviewing/revising/approved/locked）
- [ ] 12.2 实现章节状态流转逻辑与状态日志（ChapterStatusLog）
- [ ] 12.3 实现批注系统 Annotation（创建/解析/解决/跟踪）
- [ ] 12.4 实现修订对比视图（Diff 展示修改前后）
- [ ] 12.5 实现章节版本管理（ChapterVersion 快照/回滚/历史）
- [ ] 12.6 实现批量章节状态管理 UI（总览表格 + 批量操作 + 完成度统计）
- [ ] 12.7 实现 AI 辅助审阅（自动标注问题点 + 修改建议）

### 第十三阶段：高级功能（含成本控制）
- [ ] 13.1 多模型切换与配置
- [ ] 13.2 写作统计与 Token 成本监控仪表盘
- [ ] 13.3 成本控制与 Token 预算管理系统（预算设置/用量预警/优化建议）
- [ ] 13.4 导出功能（TXT/PDF/EPUB/HTML）
- [ ] 13.5 全文搜索（FTS5）
- [ ] 13.6 章节版本快照与回滚
- [ ] 13.7 数据备份与恢复
